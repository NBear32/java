...
class GenericMethods { // 일반 클래스 안에 정의한 제네릭메서드 예제
    ...
    public <T> Boolean method2 (T t1, T t2) { // 한 개의 제네릭 타입
        return t1.equals(t2);
    }
    public <K, V> void method3 (K k, V v) { // 두 개의 제네릭 타입을 사용
        System.out.println(k + " : " + v);
    }
    public <A, B, C> void method4 (A a, B b, C c) { // 세 개의 제네릭 타입을 사용
        System.out.println(a + " : " + b + " : " + c);
    }
    // 제너릭 메서드 내부에서 사용할 수 있는 메서드
    // 반드시 제네릭 T 의 위치에 올 수 있는 모든 클래스들이 사용 가능한 메서드만 쓸 수 있음
    public <T> void method5(T t) {
        // System.out.println(t.length); // 오류, String 클래스라고 단정할 수 없음
        System.out.println(t.equals("안녕")); // 모든 클래스의 부모인 Object 의 메서드는 가능하다
    }
}
public class GenericMethod {
    public static void main(String[] args) {
        ...
        String str2 = gm.method1("안녕"); // <String> 생략 (앞뒤정보로 유추 가능하기 때문)
